...
describe("Specification Cases: node (dist/fetch-json.js)", () => {
  ...
  describe("Module fetch-json", () => {
    it("follows semantic versioning", () => {
      ...
      const actual = {
        version: fetchJson.version,
        valid: semVerPattern.test(fetchJson.version),
      };
    ...
    it("contains an array of spaceships", (done) => {
      ...
      const handleData = (data) => {
        const actual = {
          ...
          class: typeof data.results[0].starship_class,
        };
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////
  console.log(fetchJson.setDefaults);
  fetchJson.setDefaults({
    baseURL: "https://httpbin.org/",
  });
  describe("GET response returned by httpbin.org, this time with default setting", () => {
    it("contains empty params when none are supplied", (done) => {
      const url = "/get";
      const handleData = (data) => {
        const actual = { params: data.args };
    ...
    it("contains the params from the URL query string", (done) => {
      const url = "/get?planet=Jupiter&position=5";
      const handleData = (data) => {
        const actual = toPlainObj(data.args);
        const expected = { planet: "Jupiter", position: "5" };
        assert.deepEqual(actual, expected);
    ...
    it("still work if we supply the absolute URL", (done) => {
      const url = "https://httpbin.org/get?planet=Jupiter&position=5";
      const handleData = (data) => {
        const actual = toPlainObj(data.args);
        const expected = { planet: "Jupiter", position: "5" };
  ...
  describe("Response returned by httpbin.org for a planet (object literal), this time with a default setting", () => {
    it("from a POST contains the planet (JSON)", (done) => {
      const url = "/post";
      const resource = { name: "Mercury", position: 1 };
      const handleData = (data) => {
    ...
    it("still work if supply an absolute URL", (done) => {
      const url = "https://httpbin.org/post";
      const resource = { name: "Mercury", position: 1 };
      const handleData = (data) => {
        const actual = {
    ...
    it("for status 500 throws exception in strict errors mode", (done) => {
      ...
      const handleError = (error) => {
        const actual = {
          error:
            error instanceof
            (typeof window === "object" ? window.Error : Error),
          name: error.name,
  ...
  describe("Function fetchJson.enableLogger()", () => {
    ...
    it("passes a timestamp, methed, and URL to a custom logger on GET", (done) => {
      ...
      const customLogger = (...logValues) => {
        ...
        assert.deepEqual(actual, expected);
        if (logValues[index.http] === "request") awaitingRequest = false;
        else done(fetchJson.enableLogger(false));
      };
